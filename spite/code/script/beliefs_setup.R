...
`%ni%` <- Negate(`%in%`)
dat<-read.csv("pub_git/spite/data/data.csv")
vars <- c("sndisadv", "snadadv", "snaltrusim", "snspite", "snegalitarian")
...
for (k in vars){
  ...
  base=stan("code/models/b25_ag_full.stan", data = dl, cores = 4, chains = 4)
  
  saveRDS(base, paste0("data/_",k,"_gp_aggeregate_snAge.RDS"))
...
# }
# 
# vars <- c("sndisadv", "snadadv", "snselfish", "snaltrusim", "snspite", "snegalitarian")
# 
# for (k in vars){
#   
#   d <- data.frame(
...
#   
#   #Run Models
#   base=stan("code/models/agg_gp_b25_full.stan", data = dl, cores = 4)
#   saveRDS(base, paste("data/",k,"ALL_AGAGE.RDS"))
...
for (k in vars){
  ...
  saveRDS(base, paste0("data/_",k,"_IRT_snAG.RDS"))
...
for (k in vars){
  ...
  saveRDS(base, paste0("data/_",k,"_years_before_sn25AG.RDS"))
