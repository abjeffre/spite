...
parameters{
  ...
  //Interaction
  matrix[6,2] zz2;
  vector<lower=0>[6] Sigma_zX2;
  cholesky_factor_corr[6] L_RhozX2;
...
model{
  ...
  to_vector( zz ) ~ normal( 0 , 1 );
  	...
  	//linear
  L_RhozX2 ~ lkj_corr_cholesky( 2 );
  Sigma_zX2 ~ exponential( 1 );
  to_vector( zz2 ) ~ normal( 0 , 1 );


 for(i in 1:N){
    p[i] = aV[village[i]]*sigma_V +
    aX[C[i]]*sigma_X +
    bz[S[i], C[i]] +
    bz2[S[i], C[i]]*IN[i] +
    kA[C[i]][AGE[i]] + 
